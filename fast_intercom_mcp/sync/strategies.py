...
class FullThreadSyncStrategy:
    ...
    async def sync_period(self, start_date: datetime, end_date: datetime) -> SyncStats:
        ...
        await self._update_progress("fetch", 0, len(search_conversations), 
        ...
        await self._update_progress("merge", 0, len(complete_conversations), "Merging conversation data...")
        
        merged_conversations = self._merge_conversation_data(search_conversations, complete_conversations)
        
        # Phase 4: Store in database
        await self._update_progress("store", 0, len(merged_conversations), "Storing conversations...")
        ...
        await self._update_progress("complete", 1, 1, 
    ...
    def _merge_conversation_data(self, search_results: List[Conversation], 
        ...
        for conv in search_results:
            if conv.id not in seen_ids:
                ...
                merged.append(conv)
                seen_ids.add(conv.id)
                logger.warning(f"Using search data for conversation {conv.id} - complete thread fetch may have failed")
...
class IncrementalSyncStrategy:
    ...
    async def sync_since(self, since_timestamp: datetime, 
                        until_timestamp: Optional[datetime] = None) -> SyncStats:
...
class SmartSyncStrategy:
    ...
    async def sync_period(self, start_date: datetime, end_date: datetime, 
        ...
        if sync_decision["strategy"] == "full":
            return await self.full_strategy.sync_period(start_date, end_date)
        elif sync_decision["strategy"] == "incremental":
    ...
    async def _analyze_sync_requirements(self, start_date: datetime, 
        ...
        sync_state = self.db.check_sync_state(start_date, end_date, 
        ...
        if not existing_conversations:
            ...
            return {
                "strategy": "full",
                "reason": "No existing data for period"
            }
        elif sync_state["sync_state"] == "stale":
