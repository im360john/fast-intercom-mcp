...
from .state import ConversationSyncTracker, SyncStatus, SyncType, ConversationSyncState
...
class SyncStateManager:
    ...
    async def create_sync_plan(self, 
        ...
        full_sync_needed = [conv_id for conv_id in full_sync_needed 
                          if conv_id not in in_progress and conv_id not in failed_recently]
        incremental_sync_needed = [conv_id for conv_id in incremental_sync_needed 
                                 if conv_id not in in_progress and conv_id not in failed_recently]
        ...
        logger.info(f"Sync plan created: {len(plan.full_sync_needed)} full, "
    ...
    async def execute_sync_plan(self, plan: SyncPlan, 
                              progress_callback: Optional[callable] = None) -> SyncExecutionResult:
        ...
        logger.info(f"Executing sync plan for {len(plan.full_sync_needed + plan.incremental_sync_needed)} conversations")
        ...
        for i, conv_id in enumerate(plan.full_sync_needed):
            try:
                if progress_callback:
                    await progress_callback(f"Full sync {i+1}/{len(plan.full_sync_needed)}: {conv_id}")
        ...
        for i, conv_id in enumerate(plan.incremental_sync_needed):
            try:
                if progress_callback:
                    await progress_callback(f"Incremental sync {i+1}/{len(plan.incremental_sync_needed)}: {conv_id}")
        ...
        logger.info(f"Sync plan executed: {conversations_synced} synced, "
    ...
    async def _execute_full_sync(self, conversation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.sync_tracker.mark_sync_completed(
                conversation_id, SyncType.FULL, started_at, 
    ...
    async def _execute_incremental_sync(self, conversation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing_conversation:
                ...
                new_messages = [msg for msg in current_conversation.messages 
            ...
            # Mark as completed
            self.sync_tracker.mark_sync_completed(
                conversation_id, SyncType.INCREMENTAL, started_at,
    ...
    def get_sync_health_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'sync_coverage_percentage': round((full_synced / total_conversations) * 100, 1) if total_conversations > 0 else 0,
            'error_rate_percentage': round((failed / total_conversations) * 100, 1) if total_conversations > 0 else 0,
            'recent_activity': {
    ...
    def _generate_health_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if stats.get('avg_duration_seconds', 0) > 30:
            recommendations.append("Consider optimizing sync performance - operations are taking longer than expected")
        
        total_conversations = stats.get('total_conversations', 0)
        full_synced = stats.get('full_synced', 0)
        
        if total_conversations > 0 and (full_synced / total_conversations) < 0.8:
            recommendations.append("Many conversations haven't been fully synced - consider running a comprehensive sync")
        
        if stats.get('recent_failures', 0) > stats.get('recent_full_syncs', 0) + stats.get('recent_incremental_syncs', 0):
